...
function mockResponse(init) {
    ...
    return new Response(body, { status, headers: h, url } );
}
...
describe("HttpClient2 CI", () => {
    ...
    it("handles null/empty context gracefully", () => {
        const c = new HttpClient({} , FULL_CONFIG);
        expect(c).toBeDefined();
    });
...
describe("HttpClient2 Error Classification", () => {
    it("classifies 400 Bad Request", () => {
        const r = classifyError({ response: { status: 400 } } );
        expect(r.type).toBe("badRequest");
        expect(r.retryable).toBe(false);
    });

    it("classifies 401 Unauthorized", () => {
        const r = classifyError({ response: { status: 401 } } );
        expect(r.type).toBe("unauthorized");
        expect(r.isAuth).toBe(true);
    });

    it("classifies 403 Forbidden", () => {
        const r = classifyError({ response: { status: 403 } } );
        expect(r.type).toBe("forbidden");
        expect(r.isAuth).toBe(true);
    });

    it("classifies 404 Not Found", () => {
        const r = classifyError({ response: { status: 404 } } );
        expect(r.type).toBe("notFound");
    });

    it("classifies 405, 409, 422", () => {
        expect(classifyError({ response: { status: 405 } } ).type).toBe("methodNotAllowed");
        expect(classifyError({ response: { status: 409 } } ).type).toBe("conflict");
        expect(classifyError({ response: { status: 422 } } ).type).toBe("unprocessableEntity");
    });
    ...
    it("classifies 5xx server errors", () => {
        expect(classifyError({ response: { status: 500 } } ).type).toBe("internalServerError");
        expect(classifyError({ response: { status: 502 } } ).type).toBe("badGateway");
        expect(classifyError({ response: { status: 503 } } ).type).toBe("serviceUnavailable");
        expect(classifyError({ response: { status: 504 } } ).type).toBe("gatewayTimeout");
    });

    it("classifies response status in default ranges", () => {
        const r4 = classifyError({ response: { status: 418 } } );
        expect(r4.type).toBe("clientError");
        expect(r4.status).toBe("clientError");
        const r5 = classifyError({ response: { status: 599 } } );
