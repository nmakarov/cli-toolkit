...
describe("Logger CI", () => {
    ...
    it("formatLog prints the task tag, not the process name", () => {
        const line = formatLog(
            { level: "info", name: "v2intake-host", task: "intakeCycle:bright", message: "harvesting" },
            { timestamp: false, showLevel: true, color: false },
        );
        expect(line).toMatch(/^INFO\s+\[intakeCycle:bright\] harvesting$/);
        expect(line).not.toContain("v2intake");
        const progress = formatLog(
            {
                level: "progress",
                name: "v2intake-host",
                task: "intakeCycle:bright",
                prefix: "bright/media",
                count: 10,
                total: 20,
                message: "loading",
        ...
        expect(progress).toContain("[intakeCycle:bright]");
        expect(progress).not.toContain("v2intake");
        expect(progress).toContain("bright/media");
    });

    it("child() tags lines without mutating the parent logger", () => {
        const logger = new Logger(createTestContext(), { name: "v2intake-host", route: "console", showLevel: false });
        ...
        expect(childLine).not.toContain("v2intake");
        expect(parentLine).not.toContain("retroBackfill");
        expect(parentLine).not.toContain("v2intake");
    });
});
