...
- **Group caps**: optional max **alive** peers per `service_group` (peers stale after `--runnerHeartbeatStaleMs`). The library default is unlimited (`0`) for every group. A consuming app injects per-group caps with `setGroupMaxInstancesDefaults`. Override one runner with `--runnerGroupMaxInstances` (0 = unlimited). Use `--runnerEnforceMaxInstances=false` to warn instead of exiting when over limit.
...
- `setLoggerTaskLabelResolver(fn)` — optional product hook for log tags when `params.logTask` is absent (default is `name:source`).
- `setGroupMaxInstancesDefaults(map)` — optional per-group alive-instance caps (default unlimited).
