...
function readParamObject(params: any): Record<string, any> {
    if (!params) return {};
...
function buildItemLabel(row: TaskRowLite): string {
...
async function pickAction(rows: TaskRowLite[]): Promise<
    | { type: "toggle"; id: string; paused: boolean }
    | { type: "pauseAll" }
    | { type: "unpauseAll" }
    | { type: "quit" }
    | null
...
const flow = async (context: any) => {
    ...
    while (true) {
        ...
        const rows = (await query) as TaskRowLite[];
