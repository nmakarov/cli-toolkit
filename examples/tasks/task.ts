...
function parseTaskParams(paramsRaw: string | undefined): Record<string, any> | null {
...
function validateSchedule(schedule: string | undefined): string | null {
...
function parseExtraKeyValueTokens(tokens: string[]): Record<string, any> {
...
function formatTaskResults(taskName: string, results: any): string {
...
async function pickTaskForDelete(tasks: any[]): Promise<string | null> {
...
const flow = async (context: any) => {
    ...
    if (operation === "delete") {
        let deleteId: string | null = id || null;
