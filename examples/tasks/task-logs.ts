...
function hasErrorLevel(entry: LogEntry): boolean {
    const level = typeof entry?.payload?.level === "string" ? entry.payload.level.toLowerCase() : "";
    return level === "error" || level === "fatal";
}

function toOneLineMessage(entry: LogEntry): string {
    const text = messageOf(entry).replace(/\s+/g, " ").trim();
    if (!text) return "-";
    return text.length > 140 ? `${text.slice(0, 137)}...` : text;
}

async function pickErrorForTrace(entries: LogEntry[]): Promise<string | null> {
...
const flow = async (context: any) => {
    ...
    const entries = asArray(data) as LogEntry[];
    ...
    const errorEntries = asArray(errorData) as LogEntry[];
    ...
    if (mode === "latestHarvest") {
        ...
        const latestOpid = harvestEntries[harvestEntries.length - 1].opid as string;
