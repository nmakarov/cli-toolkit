...
async function executeManualRecovery(
    ...
    try {
        ...
        const runResult = await taskInstance.run(async (progress) => {
            await db(tasksTable).where({ id: row.id }).update({
    ...
    const { paused_at: _pausedAt, ...rowForHistory } = taskForRun ;
...
const flow = async (context) => {
    ...
    const row = await dbCall(tasksTable).where({ id }).first() ;
    if (!row) {
