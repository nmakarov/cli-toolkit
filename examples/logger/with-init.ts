...
const flow = async (context) => {
    ...
    logger.info(`Mode: ${(logger as any).options?.mode || "unknown"}`);
    logger.info(`Route: ${(logger as any).options?.route || "unknown"}`);
    ...
    if (Object.keys(figuredParams).length > 0) {
        ...
        for (const [key, param] of Object.entries(figuredParams) as [string, { value: any; source: "cli" | "options" | "default" }][]) {
