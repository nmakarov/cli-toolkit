...
const runCommandAndCapture = async (variant) => {
...
const showCapturedExampleResult = async (group, variant) => {
    let output = "";
    let hasError = false;
...
const runInteractiveExample = async (group, variant) => {
    console.info("\n" + "=".repeat(60));
...
const promptForExample = async (group) => {
...
const main = async () => {
    try {
        while (true) {
            ...
            while (true) {
                const variant = await promptForExample(group );
                if (!variant) {
                    break;
                }

                if ((variant ).interactive) {
                    await runInteractiveExample(group , variant );
                } else {
                    await showCapturedExampleResult(group , variant );
                }
            }
        }
    } catch (error) {
