 ...
 * From package root:
 *
 *   npx tsx scripts/ssm/ssm-pull.ts --path /mlsfarm/tf3/
 *   npm run ssm:pull -- --path /mlsfarm/tf3/ --output dotenv --file .env.from-ssm
 *   eval "$(npx tsx scripts/ssm/ssm-pull.ts --path /mlsfarm/tf3/ --output shell)"
 */
...
function leafKey(paramName: string): string {
...
async function fetchAll(
    ...
    const outList: { name: string; value: string; type?: string }[] = [];
