 ...
 *   built-in service tasks (`SERVICE_TASK_NAMES`: ping, stop, shellCommand, …).
 ...
 *
 * @example
 * npx tsx scripts/tasks-worker.js --dbName=local --serviceGroup=harvest --queueName=tasks
 * npx tsx scripts/tasks-worker.js --dbName=local --serviceGroup=loader --allowedTasks=dummyLoad
 */
...
const flow = async (context) => {
    ...
    context.logger.info?.(
        `[tasks-worker] host=${os.hostname()} queue=${queueName} serviceGroup=${sg} db=${db.config?.name ?? "?"} allowedTasks=[${resolvedAllowed.join(", ")}] serviceDefaults=[${SERVICE_TASK_NAMES.join(", ")}] maxInstances=${runnerGroupMaxInstances ?? "default"}`
    );

    await ensureTaskTables(context, { queueName, recreate: !!recreateTaskTables });

    context.registerCleanup(async (ctx) => {
        const reg = ctx.servicesRegistry;
        if (reg?.rowId) {
