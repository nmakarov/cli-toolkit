 ...
 *
 * Example:
 *   npx tsx scripts/tasks-monitor.ts --dbName=local --table=tasks --pollMs=1500
 *   npx tsx scripts/tasks-monitor.ts --maintenanceIntervalMs=10000 --maintenancePingTimeoutMs=1000 --maintenanceRegistry=true
...
import type { ServicesRegistryRow } from "../src/tasks/servicesRegistry.js";
...
function taskParamsObject(row: TaskRow): Record<string, unknown> {
    const p = row.params as unknown;
...
function parseTaskResults(results: unknown): Record<string, unknown> | null {
    ...
    if (typeof results === "object") return results as Record<string, unknown>;
    return null;
}
 ...
 */
function formatProgressPayloadForMonitor(pl: Record<string, unknown>): string | null {
...
function formatLogRecordLine(rec: Record<string, unknown>): string {
    const ts = typeof rec.ts === "string" ? rec.ts : "?";
    const pl = rec.payload as Record<string, unknown> | string | null | undefined;
    let msg = "";
    let lv = "";
    if (pl && typeof pl === "object") {
        const recPl = pl as Record<string, unknown>;
        const progressMsg = formatProgressPayloadForMonitor(recPl);
        if (progressMsg != null) {
            msg = progressMsg;
            lv = typeof recPl.level === "string" ? recPl.level : "progress";
        } else if (typeof recPl.message === "string") {
            msg = recPl.message;
            if (typeof recPl.level === "string") lv = recPl.level;
        } else {
 ...
 */
async function fetchLogRecordsForMonitor(
    context: any,
    queueName: string,
    row: TaskRow,
    opts: { tail: number; afterTs: string | null }
...
function formatTs(v: Date | string | null | undefined): string {
    if (v == null) return "-";
    const d = typeof v === "string" ? new Date(v) : v;
    return Number.isNaN(d.getTime()) ? String(v) : d.toISOString().replace("T", " ").slice(0, 19);
}

function oneLine(s: string | null | undefined, max = 64): string {
    if (s == null || s === "") return "-";
...
function padOrTruncateCell(s: string, w: number): string {
    const t = s.replace(/\s+/g, " ").trim();
...
function formatNextRunCell(row: TaskRow, now: Date): string {
...
function taskParamsSourceResourceDisplay(row: TaskRow): { source: string; resource: string } {
 ...
 */
function computeTasksQueueColumnWidths(
    rows: TaskRow[],
    now: Date,
    peaks?: { svc: number; started: number }
): {
    name: number;
    status: number;
    svc: number;
...
function formatTasksQueueTableLine(row: TaskRow, w: ReturnType<typeof computeTasksQueueColumnWidths>, now: Date): string {
...
function formatTasksQueueHeader(w: ReturnType<typeof computeTasksQueueColumnWidths>): string {
...
type RegistryRowWithRunning = ServicesRegistryRow & { runningCount: number };

/**
 * Count `tasks` rows in `running` status for this registry row.
 * Mirrors {@link claimNextRunnableTask}: each column matches if the task value is NULL (any runner) or equals the registry value.
 ...
 */
async function countRunningTasksForRegistry(db: any, tasksTable: string, r: ServicesRegistryRow): Promise<number> {
    const row = await db(tasksTable)
        ...
        .count("* as count")
        .first();
    return Number((row as { count?: string })?.count ?? 0);
}

/** Instance column: numeric instance + short row id (stable across similar service_name). */
function formatRegistryInstanceCell(r: ServicesRegistryRow): string {
...
function computeRegistryColumnWidths(rows: RegistryRowWithRunning[]): {
    g: number;
    sn: number;
    inst: number;
    srv: number;
...
function formatRegistryTableLine(
    r: RegistryRowWithRunning,
    w: { g: number; sn: number; inst: number; srv: number; run: number; pid: number }
): string {
...
function formatRegistryHeader(w: { g: number; sn: number; inst: number; srv: number; run: number; pid: number }): string {
...
type RegistryInfoPanel =
    | null
    | { phase: "loading" }
    | { phase: "error"; message: string }
    | { phase: "ok"; title: string; lines: string[] };

/** Aggregated row from `${queue}_history`: group by task name + params source/resource. */
type HistoryGroupAgg = {
    name: string;
    source: string;
    resource: string;
    count: number;
    repServer: string | null;
};

function historyGroupKey(g: Pick<HistoryGroupAgg, "name" | "source" | "resource">): string {
    return `${g.name}\x1f${g.source}\x1f${g.resource}`;
}

function syntheticTaskRowForHistoryGroup(g: HistoryGroupAgg): TaskRow {
...
async function loadHistoryGroups(db: any, historyTable: string): Promise<HistoryGroupAgg[]> {
...
async function fetchLatestHistoryRowForGroup(db: any, historyTable: string, g: HistoryGroupAgg): Promise<TaskRow | null> {
...
function linesForHistoryResultPreview(row: TaskRow | null): string[] {
...
function computeHistoryGroupColumnWidths(rows: HistoryGroupAgg[]): { nm: number; src: number; res: number; cnt: number } {
    const minNm = 6;
    const minSrc = 6;
    const minRes = 6;
    const minCnt = 5;
...
function formatHistoryGroupLine(r: HistoryGroupAgg, w: ReturnType<typeof computeHistoryGroupColumnWidths>): string {
...
function formatHistoryGroupHeader(w: ReturnType<typeof computeHistoryGroupColumnWidths>): string {
...
function buildInfoTaskResultText(row: TaskRow): { title: string; lines: string[] } {
    ...
    if (row.success === false) {
        const err = parsed && typeof parsed === "object" && "error" in parsed ? String((parsed as { error?: unknown }).error) : JSON.stringify(parsed);
...
function showRegistryPickerScreen(
    context: any,
    db: any,
    ...
    pollMs: number,
    queueName: string
): Promise<ServicesRegistryRow | null> {
    return showScreen({
        ...
        onRender: (ctx: any) => {
            ...
            const RegistryPicker = () => {
                const [rows, setRows] = useState<RegistryRowWithRunning[]>([]);
                const [lastUpdated, setLastUpdated] = useState("-");
                const [errorText, setErrorText] = useState("");
                const [infoPanel, setInfoPanel] = useState<RegistryInfoPanel>(null);
                const infoPanelRef = useRef<RegistryInfoPanel>(null);
                infoPanelRef.current = infoPanel;
                const selectedIndexRef = useRef(0);
                const itemsRef = useRef<Array<{ name: string; value: ServicesRegistryRow }>>([]);
                ...
                useEffect(() => {
                    ...
                    ctx.setAction("serviceInfo", () => {
                        const picked = itemsRef.current[selectedIndexRef.current]?.value as ServicesRegistryRow | undefined;
                        if (!picked) return;
...
function showServiceRunningScreen(db: any, tasksTable: string, reg: ServicesRegistryRow, pollMs: number): Promise<void> {
    return showScreen({
...
const LogTailPanel = memo(function LogTailPanel(props: {
    ...
    row: TaskRow;
    logPollMs: number;
    logInitialTail: number;
    onBack: () => void;
}): ReturnType<typeof h> {
    const linesAccRef = useRef<string[]>([]);
    const maxTsRef = useRef<string | null>(null);
    ...
    return showScreen({
        ...
        onRender: (ctx: any) => {
            ...
            const Watch = () => {
                ...
                const [logRow, setLogRow] = useState<TaskRow | null>(null);
                ...
                useEffect(() => {
                    ...
                    const load = async () => {
                        ...
                        try {
                            ...
                            if (!active) return;
                            setRows(loaded as TaskRow[]);
                            rowsRef.current = loaded as TaskRow[];
                            setLastUpdated(new Date().toISOString());
                            setErrorText("");
                        } catch (e: any) {
                            if (!active) return;
                ...
                const items: Array<{ name: string; value: TaskRow }> = rows.map((row) => ({
                ...
                return h(Box, { flexDirection: "column" },
                    ...
                    h(ListComponent, {
                        ...
                        onSelectionChange: (_idx, item) => {
                            const row = item?.value as TaskRow | undefined;
...
type HistoryViewState = null | { kind: "ipc"; row: TaskRow } | { kind: "json"; title: string; lines: string[] };
...
function showTasksHistoryScreen(
    context: any,
    db: any,
    queueName: string,
    historyTable: string,
    ...
    return showScreen({
        ...
        onRender: (ctx: any) => {
            ...
            const Watch = () => {
                const [groups, setGroups] = useState<HistoryGroupAgg[]>([]);
                const [lastUpdated, setLastUpdated] = useState("-");
                const [errorText, setErrorText] = useState("");
                const [view, setView] = useState<HistoryViewState>(null);
                const [selectedKey, setSelectedKey] = useState<string | null>(null);
                const selectedIndexRef = useRef(0);
                const groupsRef = useRef<HistoryGroupAgg[]>([]);
                ...
                const items: Array<{ name: string; value: HistoryGroupAgg }> = groups.map((g) => ({
                    ...
                    groups.length > 0
                        ? h(Box, { flexDirection: "column" },
                              ...
                              h(ListComponent, {
                                  ...
                                  getTitle: (it) => it.name,
                                  onSelectionChange: (_idx, item) => {
                                      const g = item?.value as HistoryGroupAgg | undefined;
...
const flow = async (context: any) => {
    ...
    const hasRegistry = await dbCall.tableExists(registryTable).catch(() => false);
    if (!hasRegistry) {
        context.logger?.warn?.(`[tasks-monitor] No table ${registryTable}; only "Tasks queue" mode is useful.`);
    }

    const hasHistory = await dbCall.tableExists(historyTable).catch(() => false);
    if (!hasHistory) {
    ...
    try {
        for (;;) {
            const items: Array<{ name: string; value: string }> = [
                ...
                ...(hasHistory ? [{ name: "Tasks history", value: "history" as const }] : []),
            ...
            if (choice === "services" && hasRegistry) {
                for (;;) {
                    const reg = await showRegistryPickerScreen(context, dbCall, registryTable, tasksTable, pollMs, table);
                    if (!reg) break;
                    await showServiceRunningScreen(dbCall, tasksTable, reg, pollMs);
                }
